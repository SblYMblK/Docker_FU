...
RUN rm -rf /var/www/*
#Помещаю из папки в докер контейнер index1.html
RUN mkdir -p /var/www/daniil/
COPY index.html /var/www/daniil
#Помещаю из папки в докер контейнер img.jpg
RUN mkdir -p /var/www/daniil/img
COPY img.jpg /var/www/daniil/img
#Задать рекурсивно на папку права владельцу - Ч,П,И группе - Ч,И остальным - Ч,И
RUN chmod -R 755 /var/www/daniil
...
RUN chown -R testdock:iris /var/www/daniil
# Заменяем значение /var/www/html в файле
RUN sed -i 's/\/var\/www\/html/\/var\/www\/daniil/g' /etc/nginx/sites-enabled/default
